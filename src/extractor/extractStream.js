...
import { extractVidnest } from "./extractVidnest.js";

export const extractStream = async ({ selectedServer, id }) => {
  // Special handling for HD-4 (Megaplay)
  ...
  // ✅ Vidnest integration
  if (selectedServer.name.toLowerCase() === "vidnest") {
    const anilistId = id.split("?ep=")[0];   // extract AniList ID
    const episode = id.split("?ep=").pop();  // extract episode
    return await extractVidnest(anilistId, episode, "hindi");
  }
